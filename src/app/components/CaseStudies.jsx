...
const caseColors = ["var(--brand-neon)", "var(--brand-sec)", "var(--brand-neon)"];
  ...
  return (
    <section className="py-24 relative overflow-hidden" style={{ background: colors.bgAlt, transition: "background-color 0.4s ease" }}>
      <div className="absolute top-0 left-0 right-0 h-px"
        style={{ background: "linear-gradient(90deg, transparent, rgba(var(--brand-neon-rgb),0.2), transparent)" }} />
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-16">
          {t.metrics.map((label, i) => (
            <motion.div
              ...
              whileHover={{ scale: 1.04, borderColor: `${i % 2 === 0 ? "var(--brand-neon)" : "var(--brand-sec)"}40` }}
            >
              <p style={{ fontFamily: "'Space Grotesk', sans-serif", color: i % 2 === 0 ? "var(--brand-neon)" : "var(--brand-sec)", fontSize: "clamp(1.8rem, 3vw, 2.5rem)", fontWeight: 700, lineHeight: 1, textShadow: `0 0 20px ${i % 2 === 0 ? "var(--brand-neon)" : "var(--brand-sec)"}50`, marginBottom: "8px" }}>
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            ...
            <div className="flex gap-6">
              ...
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full" style={{ background: "var(--brand-neon)" }} />
              ...
              <AreaChart data={chartData}>
                <defs>
                  <linearGradient id="afterGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="var(--brand-neon)" stopOpacity={0.3} />
                    <stop offset="100%" stopColor="var(--brand-neon)" stopOpacity={0} />
                  </linearGradient>
                ...
                <Tooltip contentStyle={{ background: isDark ? "rgba(5,5,5,0.9)" : "rgba(245,245,240,0.95)", border: "1px solid rgba(var(--brand-neon-rgb),0.3)", borderRadius: "12px", color: colors.text, fontFamily: "Space Grotesk" }} />
                <Area type="monotone" dataKey="before" stroke={colors.textDim} fill="url(#beforeGrad)" strokeWidth={2} />
                <Area type="monotone" dataKey="after" stroke="var(--brand-neon)" fill="url(#afterGrad)" strokeWidth={2.5}
                  style={{ filter: "drop-shadow(0 0 6px rgba(var(--brand-neon-rgb),0.6))" }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
